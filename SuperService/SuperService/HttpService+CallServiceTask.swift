...
extension HttpService {
  //获取呼叫服务任务列表
  func getCallServicelist(taskid:String, isowner:Bool = false, page:Int = 0, completionHandler:([CallServiceModel]?,NSError?) -> ()) {
  ...
  //操作呼叫服务任务追踪
  func servicetaskDetail(taskid:String,completionHandler:(TasktrackingModel?,NSError?) -> ()) {
    let url = ResourcePath.ServicetaskDetail.description.fullUrl + "/\(taskid)"
    get(url, parameters: nil) { (json, error) in
      if let error = error {
        completionHandler(nil,error)
      } else {
        if let task = json?["data"] {
          let taskhistory = TasktrackingModel(dic:task)
          completionHandler(taskhistory,nil)
        }
      }
    }
  }
