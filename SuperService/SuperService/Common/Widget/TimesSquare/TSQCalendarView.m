...
- (void)selectDate:(NSDate *)newSelectedDate
{
    // clamp to beginning of its day
    NSDate *tempDay = [self clampDate:newSelectedDate toComponents:NSCalendarUnitDay|NSCalendarUnitMonth|NSCalendarUnitYear];
  
  if ([tempDay isEqualToDate:self.startDate]) {
    return;
  }
  
    if ([self.delegate respondsToSelector:@selector(calendarView:shouldSelectDate:)] && ![self.delegate calendarView:self shouldSelectDate:tempDay]) {
        return;
    }
  
  if (self.startDate == nil) {
    self.startDate = tempDay;
    [[self cellForRowAtDate:self.startDate] selectStartColumnForDate:self.startDate];
  }else if (self.startDate != nil && self.endDate != nil) {//开始和结束时间都已经有了
      [[self cellForRowAtDate:self.startDate] unselectStartColumn];
      self.startDate = nil;
      [[self cellForRowAtDate:self.endDate] unselectEndColumn];
      self.endDate = nil;
      self.startDate = tempDay;
      [[self cellForRowAtDate:self.startDate] selectStartColumnForDate:self.startDate];
  }else {
    if ([self.startDate earlierDate:tempDay] == self.startDate) {//startDate 早于 tempDay
      [[self cellForRowAtDate:self.endDate] unselectEndColumn];
      self.endDate = tempDay;
      [[self cellForRowAtDate:self.endDate] selectEndColumnForDate:self.endDate];
    }else {
      [[self cellForRowAtDate:self.startDate] unselectStartColumn];
      self.startDate = tempDay;
      [[self cellForRowAtDate:self.startDate] selectStartColumnForDate:self.startDate];
...
//    [[self cellForRowAtDate:_startDate] selectColumnForDate:nil];
//    [[self cellForRowAtDate:tempDay] selectColumnForDate:tempDay];
//    [[self cellForRowAtDate:_endDate] selectColumnForDate:nil];
//    [[self cellForRowAtDate:tempDay] selectColumnForDate:tempDay];
  

    NSIndexPath *newIndexPath = [self indexPathForRowAtDate:tempDay];
...
//    _selectedDate = tempDay;
  
    if ([self.delegate respondsToSelector:@selector(calendarView:didSelectDate:)]) {
        [self.delegate calendarView:self didSelectDate:tempDay];
    }
}
...
- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath;
{
    ...
    if (indexPath.row > 0 || self.pinsHeaderToTop) {
        ...
        dateComponents.weekOfYear = indexPath.row - (self.pinsHeaderToTop ? 0 : 1);
...
//        [(TSQCalendarRowCell *)cell selectColumnForDate:self.selectedDate];
      ...
      if (self.startDate) {
        [(TSQCalendarRowCell *)cell selectStartColumnForDate:self.startDate];
      }else {
        [(TSQCalendarRowCell *)cell unselectStartColumn];
      }
      if (self.endDate) {
        [(TSQCalendarRowCell *)cell selectEndColumnForDate:self.endDate];
      }else {
        [(TSQCalendarRowCell *)cell unselectEndColumn];
      }
      
//        [(TSQCalendarRowCell *)cell selectColumnForDate:self.startDate];
//        [(TSQCalendarRowCell *)cell selectColumnForDate:self.endDate];
